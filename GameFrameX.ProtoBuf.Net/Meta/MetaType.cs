...
public class MetaType : ISerializerProxy
{
    ...
    private IProtoTypeSerializer BuildSerializer()
    {
        ...
        if (IsAutoTuple)
        {
            var ctor = ResolveTupleConstructor(Type, out var mapping);
            if (ctor == null)
            {
                throw new InvalidOperationException();
            }
    ...
    internal static bool GetAsReferenceDefault(RuntimeTypeModel model, Type type)
    {
        ...
        for (var i = 0; i < typeAttribs.Length; i++)
        {
            ...
            {
                if (typeAttribs[i].TryGet("AsReferenceDefault", out var tmp))
                {
                    return (bool)tmp;
                }
            }
    ...
    internal void ApplyDefaultBehaviourImpl(bool inheritPropertyInfo = true)
    {
        ...
        var typeAttribs = AttributeMap.Create(model, Type, true);
        ...
        var enumShouldUseImplicitPassThru = isEnum;
    ...
    private static void GetDataFormat(ref DataFormat value, AttributeMap attrib, string memberName)
    {
        ...
        if (attrib.TryGet(memberName, out var obj) && obj != null)
        {
    ...
    private static bool GetFieldBoolean(ref bool value, AttributeMap attrib, string memberName, bool publicOnly)
    {
        ...
        if (attrib.TryGet(memberName, publicOnly, out var obj) && obj != null)
        {
            value = (bool)obj;
            return true;
        }
    ...
    private ValueMember AddField(int fieldNumber, string memberName, Type itemType, Type defaultType, object defaultValue)
    {
        ...
        {
            var name = $"<{((PropertyInfo)mi).Name}>k__BackingField";
#if PROFILE259
        ...
        var newField = new ValueMember(model, Type, fieldNumber, backingField ?? mi, miType, itemType, defaultType, DataFormat.Default, defaultValue);
    ...
    private readonly BasicList fields = new();
