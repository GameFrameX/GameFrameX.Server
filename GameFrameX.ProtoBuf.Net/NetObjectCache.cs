...
internal sealed class NetObjectCache
{
    ...
    {
        get { return underlyingList ?? (underlyingList = new MutableList()); }
    }
    ...
    internal void SetKeyedObject(int key, object value)
    {
        if (key-- == Root)
        {
            ...
            if (rootObject != null && rootObject != (object)value)
            {
    ...
    internal int AddObjectKey(object value, out bool existing)
    {
        ...
        if (value == (object)rootObject) // (object) here is no-op, but should be
        {
