...
public sealed class ProtoReader : IDisposable
{
    ...
    private int depth, ioIndex, available;
    private long blockEnd64, dataRemaining64;
    ...
    private static void Init(ProtoReader reader, Stream source, TypeModel model, SerializationContext context, long length)
    {
        ...
        if (!source.CanRead)
        {
            throw new ArgumentException("Cannot read from stream", nameof(source));
        }
    ...
    {
        var read = TryReadUInt32VariantWithoutMoving(trimNegative, out var value);
        if (read > 0)
        {
            ioIndex += read;
            available -= read;
    ...
    {
        var read = TryReadUInt64VariantWithoutMoving(out var value);
        if (read > 0)
        {
            ioIndex += read;
            available -= read;
    ...
    private string Intern(string value)
    {
        ...
        else if (stringInterner.TryGetValue(value, out var found))
        {
            value = found;
        }
        else
    ...
    public static void EndSubItem(SubItemToken token, ProtoReader reader)
    {
        ...
        switch (reader.WireType)
        {
            ...
            default:
                if (value64 < reader.LongPosition)
                {
                    throw reader.CreateException($"Sub-message not read entirely; expected {value64}, was {reader.LongPosition}");
                }
    ...
    public static SubItemToken StartSubItem(ProtoReader reader)
    {
        ...
        switch (reader.WireType)
        {
            ...
            case WireType.String:
                var len = (long)reader.ReadUInt64Variant();
                if (len < 0)
                {
                    reader.ThrowInvalidLength(len);
                }

                var lastEnd = reader.blockEnd64;
    ...
    public int ReadFieldHeader()
    {
        ...
        if (TryReadUInt32Variant(out var tag) && tag != 0)
        {
    ...
    public bool TryReadFieldHeader(int field)
    {
        ...
        var read = TryReadUInt32VariantWithoutMoving(false, out var tag);
    ...
    public void Hint(WireType wireType)
    {
        ...
        else if (((int)wireType & 7) == (int)WireType)
        {
    ...
    {
        return ReadLengthPrefix(source, expectHeader, style, out fieldNumber, out var bytesRead);
    }
    ...
    {
        var bytes = TryReadUInt64Variant(source, out var val);
        if (bytes <= 0)
        {
            throw EoF(null);
        }
    ...
    public static int ReadLengthPrefix(Stream source, bool expectHeader, PrefixStyle style, out int fieldNumber, out int bytesRead)
    {
        ...
        var len64 = ReadLongLengthPrefix(source, expectHeader, style, out fieldNumber, out bytesRead);
    ...
    public void AppendExtensionData(IExtensible instance)
    {
        ...
        try
        {
            ...
            using (var writer = ProtoWriter.Create(dest, Model))
            {
