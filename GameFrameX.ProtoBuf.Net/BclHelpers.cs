...
public static class BclHelpers
{
    ...
    {
        new(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
        new(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
        new(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Local),
    };
    ...
    {
        var ticks = ReadTimeSpanTicks(source, out var kind);
    ...
    {
        var seconds = ToDurationSeconds(value, out var nanos);
    ...
    {
        var seconds = ToDurationSeconds(value - TimestampEpoch, out var nanos);
                ...
                switch (scale)
                {
                    ...
                    case TimeSpanScale.MinMax:
                        switch (value)
                        {
                            ...
                            default: throw new ProtoException("Unknown min/max value: " + value);
                        }
                    default:
    ...
    public static void WriteNetObject(object value, ProtoWriter dest, int key, NetObjectOptions options)
    {
        ...
        if (asReference)
        {
            var objectKey = dest.NetCache.AddObjectKey(value, out var existing);
