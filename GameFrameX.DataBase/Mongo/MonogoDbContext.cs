...
internal sealed class MongoDbContext : DBContext
{
    ...
    // {
    //     Action<BaseCacheState> action = m => { m.UpdateTime = TimeHelper.UnixTimeMilliseconds(); m.UpdateCount++; };
    //     if (typeof(T) == typeof(BaseCacheState) || typeof(T).IsSubclassOf(typeof(BaseCacheState)))
    //     {
    //         return action;
    //     }
    //     
