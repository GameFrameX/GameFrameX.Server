...
/// 随机负载均衡器
/// </summary>
public sealed class RandomLoadBalancer : BaseLoadBalancer
{
    private readonly System.Random _random = new();

    public RandomLoadBalancer() : base(LoadBalanceStrategy.Random)
    {
    }
    ...
    protected override long SelectInstanceInternal(string serverType, List<long> availableInstances, string key = null)
    {
        ...
        var index = _random.Next(activeInstances.Count);
    ...
    protected override LoadBalancingSelection SelectInstanceEnhancedInternal(LoadBalancingContext context, List<long> availableInstances)
    {
        ...
        if (selectedInstance > 0)
        {
            ...
            return LoadBalancingSelection.Success(selectedInstance, SelectionReason.Random, score, selectionTime);
        }

        return LoadBalancingSelection.Failure("Random selection failed");
    }
}