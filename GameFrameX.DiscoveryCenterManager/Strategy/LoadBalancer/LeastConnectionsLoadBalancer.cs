...
/// 最少连接数负载均衡器
/// </summary>
public sealed class LeastConnectionsLoadBalancer : BaseLoadBalancer
{
    private readonly ConcurrentDictionary<long, int> _connectionCounts = new();

    public LeastConnectionsLoadBalancer() : base(LoadBalanceStrategy.LeastConnections)
    {
    }
    ...
    protected override long SelectInstanceInternal(string serverType, List<long> availableInstances, string key = null)
    {
        ...
        return activeInstances.OrderBy(id => _connectionCounts.TryGetValue(id, out var count) ? count : 0).First();
    }
    ...
    protected override LoadBalancingSelection SelectInstanceEnhancedInternal(LoadBalancingContext context, List<long> availableInstances)
    {
        ...
        var selectedInstance = activeInstances.OrderBy(id => _connectionCounts.TryGetValue(id, out var count) ? count : 0).First();
        ...
        if (selectedInstance > 0)
        {
            ...
            return LoadBalancingSelection.Success(selectedInstance, SelectionReason.LeastConnections, score, selectionTime);
        }

        return LoadBalancingSelection.Failure("LeastConnections selection failed");
    }
    ...
    public override void UpdateInstanceMetrics(long serverInstanceId, int connections = 0)
    {
        ...
        _connectionCounts.AddOrUpdate(serverInstanceId, connections, (key, oldValue) => connections);
    }
    ...
    public override void Reset()
    {
        ...
        LogHelper.Info("最少连接数负载均衡器状态已重置");
    }
}