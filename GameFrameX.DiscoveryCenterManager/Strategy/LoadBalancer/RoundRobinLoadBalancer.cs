...
/// 轮询负载均衡器
/// </summary>
public sealed class RoundRobinLoadBalancer : BaseLoadBalancer
{
    private readonly ConcurrentDictionary<string, int> _counters = new();

    public RoundRobinLoadBalancer() : base(LoadBalanceStrategy.RoundRobin)
    {
    }
    ...
    protected override long SelectInstanceInternal(string serverType, List<long> availableInstances, string key = null)
    {
        ...
        var counter = _counters.GetOrAdd(serverType, 0);
        var selected = activeInstances[counter % activeInstances.Count];
        _counters[serverType] = (counter + 1) % activeInstances.Count;
    ...
    protected override LoadBalancingSelection SelectInstanceEnhancedInternal(LoadBalancingContext context, List<long> availableInstances)
    {
        ...
        var counter = _counters.GetOrAdd(context.ServerType, 0);
        var selected = activeInstances[counter % activeInstances.Count];
        _counters[context.ServerType] = (counter + 1) % activeInstances.Count;

        if (selected > 0)
        {
            var score = CalculateInstanceScore(selected, context);
            var selectionTime = (long)System.Diagnostics.Stopwatch.GetTimestamp();

            return LoadBalancingSelection.Success(selected, SelectionReason.RoundRobin, score, selectionTime);
        }

        return LoadBalancingSelection.Failure("RoundRobin selection failed");
    }
