...
namespace GameFrameX.Utility
{
    ...
    public sealed class IllegalWordDetection
    {
        ...
        /// 存了所有的长度大于1的敏感词汇
        /// </summary>
        static readonly HashSet<string> WordsSet = new HashSet<string>();

        /// <summary>
        /// 存了某一个词在所有敏感词中的位置，（超出8个的截断为第8个位置）
        /// </summary>
        static readonly byte[] FastCheck = new byte[char.MaxValue];

        /// <summary>
        /// 存了所有敏感词的长度信息，“Key”值为所有敏感词的第一个词，敏感词的长度会截断为8
        /// </summary>
        static readonly byte[] FastLength = new byte[char.MaxValue];

        /// <summary>
        /// 保有所有敏感词汇的第一个词的记录，可用来判断是否一个词是一个或者多个敏感词汇的“第一个词”，且可判断以某一个词作为第一个词的一系列的敏感词的最大的长度
        /// </summary>
        static readonly byte[] StartCache = new byte[char.MaxValue];

        static char[] _dectectedBuffer = null;

        /// <summary>
        /// 忽略的敏感词
        /// </summary>
        static readonly string SkipList =
            " \t\r\n~!@#$%^&*()_+-=【】、{}|;':\"，。、《》？αβγδεζηθικλμνξοπρστυφχψωΑΒΓΔΕΖΗΘΙΚΛΜΝΞΟΠΡΣΤΥΦΧΨΩ。，、；：？！…—·ˉ¨‘’“”々～‖∶＂＇｀｜〃〔〕〈〉《》「」『』．〖〗【】（）［］｛｝ⅠⅡⅢⅣⅤⅥⅦⅧⅨⅩⅪⅫ⒈⒉⒊⒋⒌⒍⒎⒏⒐⒑⒒⒓⒔⒕⒖⒗⒘⒙⒚⒛㈠㈡㈢㈣㈤㈥㈦㈧㈨㈩①②③④⑤⑥⑦⑧⑨⑩⑴⑵⑶⑷⑸⑹⑺⑻⑼⑽⑾⑿⒀⒁⒂⒃⒄⒅⒆⒇≈≡≠＝≤≥＜＞≮≯∷±＋－×÷／∫∮∝∞∧∨∑∏∪∩∈∵∴⊥∥∠⌒⊙≌∽√§№☆★○●◎◇◆□℃‰€■△▲※→←↑↓〓¤°＃＆＠＼︿＿￣―♂♀┌┍┎┐┑┒┓─┄┈├┝┞┟┠┡┢┣│┆┊┬┭┮┯┰┱┲┳┼┽┾┿╀╁╂╃└┕┖┗┘┙┚┛━┅┉┤┥┦┧┨┩┪┫┃┇┋┴┵┶┷┸┹┺┻╋╊╉╈╇╆╅╄";

        static readonly BitArray SkipBitArray = new BitArray(char.MaxValue);

        /// <summary>
        /// 保有所有敏感词汇的最后一个词的记录，仅用来判断是否一个词是一个或者多个敏感词汇的“最后一个词”
        /// </summary>
        private static readonly BitArray EndCache = new BitArray(char.MaxValue);
        ...
        static unsafe void InnerInitBytes(byte[] data, int badIdx = 1)
        {
        ...
        static unsafe void InnerInit(string[] badwords)
        {
        ...
        static unsafe string OriginalToLower(string text)
        {
        ...
        static unsafe bool EnsuranceLower(string text)
        {
            fixed (char* newText = text)
            {
                char* itor = newText;
        ...
        public static unsafe string Filter(string text, char mask = '*')
        {
        ...
        public static unsafe bool DetectIllegalWords(string text, bool returnWhenFindFirst, out Dictionary<int, int> findResult)
        {
            ...
            if (text.Length > bufferLength) _dectectedBuffer = new char[bufferLength << 1];
